    ...
    : buffer(4096), m_server(server), m_remote_endpoint(std::move(endpoint)), m_logged(true)
{
    ...
    for (auto const &entity : gObjectsSystem->GetEntities()) {
        ...
        this->send(new PacketServerEntityCreate(transform.type, EntityTeam::PLAYER, transform.mobType, entity,
                                                transform.posX, transform.posY, 100));
    }
    ...
    m_server->broadcast(
        new PacketServerEntityCreate(EntityType::PLAYER, EntityTeam::PLAYER, MobType::NONE, m_entity, 50, 50, 100));
}
